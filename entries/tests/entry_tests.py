...
class EntryTests(APITestCase):
    ...
    def test_list_entries(self):
        ...
        self.assertEqual(set(response.data['results'][0].keys()),
            set(['author', 'entry_date', 'entry_timezone',
                'start_time', 'milestone_time']))
    ...
    def test_retrieve_entry(self):
        ...
        self.assertEqual(set(response.data.keys()),
            set(['author', 'entry_date', 'entry_timezone', 'start_time',
                    'milestone_time', 'finish_time', 'milestone_word_count',
                    'words', 'word_count', 'created_date', 'modified_date']))
    ...
    def test_update_entry(self):
        ...
        entry = Entry(author=user, entry_date=today, words='blah', word_count=1, milestone_word_count=750)
        entry.save()
    ...
    def test_update_entry_attempt_date_change(self):
        ...
        user = get_user_model().objects.get(username=data['author'])
        entry = Entry(author=user, entry_date=today, words='blah',
            word_count=1, milestone_word_count=750)
        entry.save()
    ...
    def test_update_entry_not_today(self):
        ...
        user = get_user_model().objects.get(username=data['author'])
        entry = Entry(author=user, entry_date=tomorrow, words='blah',
            word_count=1, milestone_word_count=750)
        entry.save()
