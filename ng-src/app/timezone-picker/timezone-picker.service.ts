...
export class TimezonePickerService {
  ...
  iso2country(iso: string): string {
    return this.countryList[iso] ? this.countryList[iso] : iso;
  }
  ...
  getCountries(): string[] {
    const res: string[] = [];
    for (const prop of Object.keys(this.countryList)) {
      res.push(prop);
    }
    return res;
  }
  ...
  getZones(): Timezone[] {
    const res = [];
    for (const prop of Object.keys(this.zones)) {
      res.push({
        iso: prop,
        zones: this.zones[prop]
      });
    }
    return res;
  }
